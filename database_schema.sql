...
-- =====================================================================
-- CATALOGO DE CORREOS INSTITUCIONALES (WHITELIST)
-- =====================================================================
...
-- =====================================================================
-- FUNCION GENERICA PARA actualizaciones de timestamp
-- =====================================================================
...
-- =====================================================================
-- REGLA DE NEGOCIO: SOLO CORREOS DE CATALOGO PUEDEN REGISTRARSE
-- =====================================================================
...
BEGIN
    ...
    IF v_catalogo_id IS NULL THEN
        RAISE EXCEPTION 'El correo % no esta en el catalogo institucional', NEW.correo_institucional;
    END IF;

    IF NOT v_habilitado THEN
        RAISE EXCEPTION 'El correo % esta deshabilitado para registro', NEW.correo_institucional;
    END IF;
...
BEGIN
    ...
    IF v_tipo_publicacion_id IS NULL THEN
        RAISE EXCEPTION 'No existe tipo_publicacion con codigo %', p_tipo_publicacion_codigo;
    END IF;
...
SELECT 'ING', 'Facultad de Ingenieria', s.id FROM sedes s WHERE s.codigo = 'MAIN'
ON CONFLICT (codigo) DO NOTHING;
...
SELECT 'SIS', 'Ingenieria de Sistemas', f.id FROM facultades f WHERE f.codigo = 'ING'
ON CONFLICT (codigo) DO NOTHING;

INSERT INTO carreras (codigo, nombre, facultad_id)
SELECT 'IND', 'Ingenieria Industrial', f.id FROM facultades f WHERE f.codigo = 'ING'
ON CONFLICT (codigo) DO NOTHING;
...
VALUES
    ...
    ('academica', 'Academica', 'Avisos academicos y tareas'),
...
-- 2) Crear publicacion general o por carrera
-- SELECT fn_crear_publicacion(1, 'Bienvenida', 'Hola comunidad universitaria', 'general', NULL, NULL, 'general', TRUE, FALSE);
-- SELECT fn_crear_publicacion(1, 'Aviso SIS', 'Reunion de laboratorio', 'carrera', (SELECT id FROM carreras WHERE codigo='SIS'), 2, 'academica', TRUE, FALSE);
...
-- SELECT * FROM fn_enviar_mensaje_directo(1, 2, 'texto', 'Hola, como vas?', NULL, '{"origen":"app_android"}'::jsonb);

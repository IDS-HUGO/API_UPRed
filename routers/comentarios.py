...
def crear_comentario(
    ...
    if publicacion.autor_id != current_user.id:
        ...
        try:
            ...
            if not tokens:
                logger.warning(
                    "Sin dispositivos activos para push nuevo_comentario autor_publicacion=%s comentario_id=%s",
                ...
                for token in tokens:
                    ...
                    if sent:
                        sent_count += 1
                logger.info(
                    "Push nuevo_comentario autor_publicacion=%s comentario_id=%s enviados=%s total_tokens=%s",
                    publicacion.autor_id,
                    nuevo_comentario.id,
                    sent_count,
                    len(tokens),
